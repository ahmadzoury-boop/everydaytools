(() => {
  ...
  const STORAGE_KEY = "toolstack_hiit_v1";
  ...
  function applyInputsToCfg() {
    ...
    cfg.workSec = clampInt(els.workSec.value, 5, 3600, 30);
    cfg.restSec = clampInt(els.restSec.value, 0, 3600, 15);
  ...
  function render() {
    ...
    document.title = `${formatMMSS(remainingSec)} — HIIT (${phaseName})`;
  }
  ...
  function startTick() {
    ...
    tickHandle = setInterval(() => {
      ...
      if (diff === 0) {
        ...
        endsAt = null;
        stopTick();
        advance(); // move to next phase/round
        return;
      }
      render();
    }, 200);
  ...
  function advance() {
    // Called when a phase finishes (or on skip)
    ensureAudioUnlocked();
  ...
  function start() {
    ...
    if (phase === "ready") {
      applyInputsToCfg();
      // show work time but don't start until user presses start
      remainingSec = cfg.workSec;
      render();
    }
    if (phase === "done") {
  ...
  function init() {
    ...
    els.presets.forEach(btn => {
      btn.addEventListener("click", () => {
        ensureAudioUnlocked();
        const r = Number(btn.getAttribute("data-rounds") || "8");
        const w = Number(btn.getAttribute("data-work") || "30");
        const rs = Number(btn.getAttribute("data-rest") || "15");

        els.rounds.value = r;
        els.workSec.value = w;
        els.restSec.value = rs;
        applyInputsToCfg();
        reset();
      });
    });
